<template>
    <div class="sample-graph maps-container d-flex flex-column flex-shrink-1">
        <div class="maps-header" style="text-align:left;">
          <div>
            <span class="option-map" style="cursor:pointer;" v-on:click="setSelectMap(0)" v-bind:class="{'active':selectMap==0}">GIS图</span>|
            <span class="option-map" style="cursor:pointer;" v-on:click="setSelectMap(1)" v-bind:class="{'active':selectMap==1}">平面图</span>|
            <span class="option-map" style="cursor:pointer;" v-on:click="setSelectMap(2)" v-bind:class="{'active':selectMap==2}">模拟图</span>
          </div>
        </div>
